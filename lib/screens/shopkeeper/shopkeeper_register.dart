// lib/screens/shopkeeper/shopkeeper_register.dart
import 'dart:ui';
...
class _ShopkeeperRegisterState extends State<ShopkeeperRegister>
  ...
  Future<void> _register() async {
    ...
    final sLicense = _shopLicenseController.text.trim();
    final sHours   = _openingHoursController.text.trim();

    // Validation
    if (name.isEmpty || cnic.isEmpty || phone.isEmpty ||
        sName.isEmpty || sAddr.isEmpty || pass.isEmpty) {
      _showSnack('Name, CNIC, Phone, Shop Name, Address aur Password zaroori hain!',
          isError: true);
      return;
    }
    if (cnic.length != 15) {
      _showSnack('Complete 13-digit CNIC daalo', isError: true);
      return;
    }
    if (phone.length != 11 || !phone.startsWith('03')) {
      _showSnack('Phone 11 digits aur 03 se shuru hona chahiye', isError: true);
      return;
    }
    if (pass.length < 6) {
      _showSnack('Password kam az kam 6 characters ka hona chahiye',
          isError: true);
      return;
    }
    ...
    try {
      // Step 1: CNIC duplicate check (users table mein)
      ...
      if (existing.isNotEmpty) {
        _showSnack('Yeh CNIC already registered hai!', isError: true);
      ...
      final userId = userInsert['user_id'];

      // Step 3: shopkeepers table mein insert karo
      await Supabase.instance.client.from('shopkeepers').insert({
        'user_id':       userId,
        'shop_name':     sName,
        'shop_address':  sAddr,
        'shop_license':  sLicense.isEmpty ? null : sLicense,
        'opening_hours': sHours.isEmpty ? null : sHours,
        'rating':        0,
      });
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      body: Stack(
        children: [
          SizedBox.expand(
              child: Image.asset('assets/images/SR_bg.jpg',
                  fit: BoxFit.cover)),
          SizedBox.expand(
              child: Container(
                          ...
                          child: Container(
                            ...
                            child: Column(
                              children: [
                                ...
                                Text(
                                    'Apni details fill karo aur shuru karo',
                                    style: GoogleFonts.poppins(
                                        fontSize: 13,
                            ...
                            child: Column(
                              children: [
                                ...
                                _buildField(
                                    ...
                                    Icons.phone_outlined, '03XXXXXXXXX',
                                    keyboardType: TextInputType.number,
                                    formatters: [
        ...
        children: [
          ...
          TextField(
            ...
            decoration: InputDecoration(
              ...
              prefixIcon:
              Icon(icon, color: _accent.withValues(alpha: 0.8), size: 20),
              counterText: '',
              filled: true,
  ...
  Widget _buildPasswordField() {
    return Padding(
      ...
      child: Column(
        ...
        children: [
          Row(children: [
            const Icon(Icons.lock_outline, color: _accent, size: 13),
            const SizedBox(width: 6),
            Text('Password *',
                style: GoogleFonts.poppins(
                    fontSize: 12,
    ...
    return TextField(
      ...
      decoration: InputDecoration(
        ...
        prefixIcon: Icon(Icons.lock_outline,
            color: widget.accentColor.withValues(alpha: 0.8), size: 20),
        ...
        focusedBorder: OutlineInputBorder(
            borderRadius: BorderRadius.circular(12),
            borderSide:
            BorderSide(color: widget.accentColor, width: 1.8)),
      ),
    );
  }
}